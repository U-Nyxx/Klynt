...
- **No wallpaper blur** without SystemUI hook: `KlyntGlassView` blurs only inside target `decorView`. Apple `backboardd` blurs wallpaper across windows. Fix planned: hook `com.android.systemui` (LSPosed) or Magisk RRO.
