package com.unyxx.act.xposed.hooks.generic
...
/**
 * Generic bottom-bar pass for any scoped package without a dedicated hook.
 *
 * Under `staticScope=true` the framework injects us into exactly the packages
 * in `scope.list` — no manual ticking. Telegram/X keep their dedicated,
 * battle-tested paths (untouched); everything else on the static list
 * (LSPosed Manager today, future targets tomorrow) lands here.
 *
 * Heuristics mirror [com.unyxx.act.xposed.hooks.twitter.TwitterBottomNavHook]
 * deliberately (same thresholds, same order): class hints → labeled-row
 * semantics → dp-size fallback. Duplication is intentional — the dedicated
 * paths stay frozen while this one evolves.
 *
 * Safety: view-tree only, no obfuscation-sensitive names, every step guarded.
 * Worst case is a logged miss, never a crash, never a tab-less screen.
 */
object GenericBottomNavHook {
