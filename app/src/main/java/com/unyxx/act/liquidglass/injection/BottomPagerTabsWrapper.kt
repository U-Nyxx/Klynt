...
class BottomPagerTabsWrapper @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : FrameLayout(context, attrs, defStyleAttr) {

    companion object {
        private const val TAG_INJECTED = 0x7F0A0003
        private const val TAG_WRAPPER = 0x7F0A0004

        fun isInjected(view: View, pkg: String): Boolean {
            val key = "klynt_lg_pager_${pkg.replace(".", "_")}_v1"
    ...
    fun wrap(originalView: View, pkg: String): View {
        ...
        setTag(TAG_WRAPPER, "klynt_lg_pager_${pkg.replace(".", "_")}_v1")
